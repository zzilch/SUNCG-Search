...
static int grid_max_resolution = /*256*/ /*128*/ /*96*/ 64;
...
static void
RasterizeNode(R3Grid *grid, R3SceneNode *node, const R3Affine& transformation)
{
  ...
  // Rasterize references into grid
  for (int i = 0; i < node->NReferences(); i++) {
    R3SceneReference *reference = node->Reference(i);
    R3Scene *referenced_scene = reference->ReferencedScene();
    RasterizeNode(grid, referenced_scene->Root(), transformation);
  }

  // Rasterize children into grid
  for (int i = 0; i < node->NChildren(); i++) {
    R3SceneNode *child = node->Child(i);
    R3Affine child_transformation = transformation;
    child_transformation.Transform(child->Transformation());
    RasterizeNode(grid, child, child_transformation);
  }    
}
...
CreateSurfaceGrid(R3SceneNode *node, R3Box bbox, RNScalar scale)
{
  ...
  // Rasterize node
  RasterizeNode(grid, node, R3identity_affine);
...
CreatePlanarGrids(const R3Grid& surface_grid)
{
  ...
  for (int dir = RN_LO; dir <= RN_HI; dir++) {
    for (int dim = RN_X; dim <= RN_Z; dim++) {
      ...
      translation[dim] = -plane.D();
      R3Affine transformation = R3identity_affine;
      transformation.Translate(translation);
      int res1 = surface_grid.Resolution(dim1);
      int res2 = surface_grid.Resolution(dim2);
      R2Box planar_box(side_box[RN_LO][dim1], side_box[RN_LO][dim2], side_box[RN_HI][dim1], side_box[RN_HI][dim2]);
      planar_grids[side].Reset(plane, transformation, res1, res2, planar_box);
...
WriteRelationships(R3Scene *scene, const char *filename)
{
  ...
  for (int i1 = 0; i1 < scene->NNodes(); i1++) {
    ...
    R3Affine transformation1 = node1->CumulativeTransformation();
    RNScalar scale1 = transformation1.ScaleFactor();

    // Get category of node1
    char category1[1024];
    strncpy(category1, name1, 1024);
    char *s = strchr(category1, '#'); if (s) *s = '\0';
    if (node1->NReferences() == 1) {
      R3SceneReference *reference = node1->Reference(0);
      R3Scene *referenced_scene = reference->ReferencedScene();
      const char *model_name = referenced_scene->Root()->Name();
      if (model_name) {
        const char *category_name = strrchr(model_name, '#');
        if (category_name) strncpy(category1, category_name+1, 1024);
        else strncpy(category1, model_name, 1024);
      }
    }

    // Only consider objects for node1 (for now)
    if (strncmp(name1, "Object#", 7)) continue;
    ...
    for (int i2 = 0; i2 < scene->NNodes(); i2++) {
      ...
      R3Affine transformation2 = node2->CumulativeTransformation();
      RNScalar scale2 = transformation2.ScaleFactor();

      // Get category of node2
      char category2[1024];
      strncpy(category2, name2, 1024);
      char *s = strchr(category2, '#'); if (s) *s = '\0';
      if (node2->NReferences() == 1) {
        R3SceneReference *reference = node2->Reference(0);
        R3Scene *referenced_scene = reference->ReferencedScene();
        const char *model_name = referenced_scene->Root()->Name();
        if (model_name) {
          const char *category_name = strrchr(model_name, '#');
          if (category_name) strncpy(category2, category_name+1, 1024);
          else strncpy(category2, model_name, 1024);
        }
      }

      // Check if nodes are in same room
      R3SceneNode *room1 = node1->Parent();
      R3SceneNode *room2 = node2->Parent();
      while (room1) { if (!strncmp(room1->Name(), "Room#", 5)) break;  room1 = room1->Parent(); }
      while (room2) { if (!strncmp(room2->Name(), "Room#", 5)) break;  room2 = room2->Parent(); }
      ...
      fprintf(fp, "%s %s  ", category1, category2);
    ...
    // Print debug message
    if (print_debug) printf("%s %s %d/%d %d\n", name1, category1, i1, scene->NNodes(), npairs);
  }
